...
const fetchFavoriteStatus = async () => {
  try {
    ...
    const response = await fetch(`http://217.71.129.139:4327/favorites/status`, {
      method: 'POST',
      headers: {
...
const toggleFavorite = async () => {
  try {
    ...
    const response = await fetch('http://217.71.129.139:4327/favorites/toggle', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
...
const handleMenuClick = async (menuItem) => {
  try {
    ...
    const response = await fetch('http://217.71.129.139:4327/diary/add', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
...
<template>
  <div class="cards-card flex flex-col shadow-md">
    <div class="recipe-info">
      <img class="recipe-image" :src="'http://217.71.129.139:4327/' + recipe.image_url" :alt="recipe.name">
