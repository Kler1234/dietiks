...
const fetchFavoriteStatus = async () => {
  try {
    ...
    const response = await fetch(`http://172.17.3.77:3000/favorites/status`, {
      method: 'POST',
      headers: {
...
const toggleFavorite = async () => {
  try {
    ...
    const response = await fetch('http://172.17.3.77:3000/favorites/toggle', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
...
const handleMenuClick = async (menuItem) => {
  try {
    ...
    const response = await fetch('http://172.17.3.77:3000/diary/add', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
