...
const fetchAutocompleteResults = async (searchTerm) => {
  try {
    const response = await fetch(`http://217.71.129.139:4101/autocomplete/${encodeURIComponent(searchTerm)}`);
    if (response.ok) {
...
const fetchCalories = async () => {
  try {
    const response = await fetch(`http://217.71.129.139:4101/product/${encodeURIComponent(productName.value)}`);
    if (response.ok) {
...
const addProduct = async () => {
  try {
    const response = await fetch('http://217.71.129.139:4101/product', {
      method: 'POST',
      headers: {
